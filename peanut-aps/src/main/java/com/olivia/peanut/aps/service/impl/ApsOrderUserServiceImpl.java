...
public class ApsOrderUserServiceImpl extends MPJBaseServiceImpl<ApsOrderUserMapper, ApsOrderUser> implements ApsOrderUserService {
  ...
  public @Override ApsOrderUserQueryListRes queryList(ApsOrderUserQueryListReq req) {
    ...
    List<ApsOrderUserDto> dataList = list.stream().map(t -> $.copy(t, ApsOrderUserDto.class))
  ...
  public @Override DynamicsPage<ApsOrderUserExportQueryPageListInfoRes> queryPageList(ApsOrderUserExportQueryPageListReq req) {
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<ApsOrderUserExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, ApsOrderUserExportQueryPageListInfoRes.class));
    ...
    List<ApsOrderUserExportQueryPageListInfoRes> listInfoRes = $.copyList(records,
        ApsOrderUserExportQueryPageListInfoRes.class);
  ...
  private MPJLambdaWrapper<ApsOrderUser> getWrapper(ApsOrderUserDto obj) {
    ...
    if (Objects.nonNull(obj)) {
      q
          ...
          .eq(StringUtils.isNoneBlank(obj.getUserName()), ApsOrderUser::getUserName,
              obj.getUserName())
          .eq(StringUtils.isNoneBlank(obj.getUserPhone()), ApsOrderUser::getUserPhone,
              obj.getUserPhone())
          .eq(Objects.nonNull(obj.getUserSex()), ApsOrderUser::getUserSex, obj.getUserSex())
          .eq(StringUtils.isNoneBlank(obj.getCountryCode()), ApsOrderUser::getCountryCode,
              obj.getCountryCode())
          .eq(StringUtils.isNoneBlank(obj.getProvinceCode()), ApsOrderUser::getProvinceCode,
              obj.getProvinceCode())
          .eq(StringUtils.isNoneBlank(obj.getCityCode()), ApsOrderUser::getCityCode,
              obj.getCityCode())
          .eq(StringUtils.isNoneBlank(obj.getAreaCode()), ApsOrderUser::getAreaCode,
              obj.getAreaCode())
          .eq(StringUtils.isNoneBlank(obj.getUserAddress()), ApsOrderUser::getUserAddress,
              obj.getUserAddress())
          .eq(StringUtils.isNoneBlank(obj.getUserRemark()), ApsOrderUser::getUserRemark,
              obj.getUserRemark())
