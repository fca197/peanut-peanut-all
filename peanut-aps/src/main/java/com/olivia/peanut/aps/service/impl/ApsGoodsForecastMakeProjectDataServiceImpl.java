...
public class ApsGoodsForecastMakeProjectDataServiceImpl extends MPJBaseServiceImpl<ApsGoodsForecastMakeProjectDataMapper, ApsGoodsForecastMakeProjectData> implements
  ...
  public @Override ApsGoodsForecastMakeProjectDataQueryListRes queryList(ApsGoodsForecastMakeProjectDataQueryListReq req) {
    ...
    List<ApsGoodsForecastMakeProjectDataDto> dataList = list.stream()
        .map(t -> $.copy(t, ApsGoodsForecastMakeProjectDataDto.class)).collect(Collectors.toList());
  ...
  public @Override DynamicsPage<ApsGoodsForecastMakeProjectDataExportQueryPageListInfoRes> queryPageList(ApsGoodsForecastMakeProjectDataExportQueryPageListReq req) {
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<ApsGoodsForecastMakeProjectDataExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, ApsGoodsForecastMakeProjectDataExportQueryPageListInfoRes.class));
      records = dataList.getRecords();
    } else {
      records = $.copyList(this.list(q),
          ApsGoodsForecastMakeProjectDataExportQueryPageListInfoRes.class);
    }

    // 类型转换，  更换枚举 等操作

    List<ApsGoodsForecastMakeProjectDataExportQueryPageListInfoRes> listInfoRes = $.copyList(
        records, ApsGoodsForecastMakeProjectDataExportQueryPageListInfoRes.class);
  ...
  private MPJLambdaWrapper<ApsGoodsForecastMakeProjectData> getWrapper(ApsGoodsForecastMakeProjectDataDto obj) {
    ...
    if (Objects.nonNull(obj)) {
      q
          .eq(Objects.nonNull(obj.getGoodsId()), ApsGoodsForecastMakeProjectData::getGoodsId,
              obj.getGoodsId())
          .eq(Objects.nonNull(obj.getMakeMonthId()),
              ApsGoodsForecastMakeProjectData::getMakeMonthId, obj.getMakeMonthId())
          .eq(StringUtils.isNoneBlank(obj.getProjectConfigCode()),
              ApsGoodsForecastMakeProjectData::getProjectConfigCode, obj.getProjectConfigCode())
          .eq(Objects.nonNull(obj.getYear()), ApsGoodsForecastMakeProjectData::getYear,
              obj.getYear())

          .eq(Objects.nonNull(obj.getFactoryId()), ApsGoodsForecastMakeProjectData::getFactoryId,
              obj.getFactoryId())
