...
@Transactional
public class ApsGoodsForecastMainMakeServiceImpl extends MPJBaseServiceImpl<ApsGoodsForecastMainMakeMapper, ApsGoodsForecastMainMake> implements
  ...
  public @Override ApsGoodsForecastMainMakeQueryListRes queryList(ApsGoodsForecastMainMakeQueryListReq req) {
    ...
    List<ApsGoodsForecastMainMakeDto> dataList = list.stream()
        .map(t -> $.copy(t, ApsGoodsForecastMainMakeDto.class)).collect(Collectors.toList());
  ...
  public @Override DynamicsPage<ApsGoodsForecastMainMakeExportQueryPageListInfoRes> queryPageList(ApsGoodsForecastMainMakeExportQueryPageListReq req) {
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<ApsGoodsForecastMainMakeExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, ApsGoodsForecastMainMakeExportQueryPageListInfoRes.class));
    ...
    List<ApsGoodsForecastMainMakeExportQueryPageListInfoRes> listInfoRes = $.copyList(records,
  ...
  public @Override void setName(
      List<? extends ApsGoodsForecastMainMakeDto> apsGoodsForecastMainMakeDtoList) {

    if (CollUtil.isEmpty(apsGoodsForecastMainMakeDtoList)) {
      return;
    }
    Set<Long> gidSet = apsGoodsForecastMainMakeDtoList.stream()
        .map(ApsGoodsForecastMainMakeDto::getGoodsId).collect(Collectors.toSet());
    Map<Long, String> gnMap = apsGoodsService.listByIds(gidSet).stream()
  ...
  private MPJLambdaWrapper<ApsGoodsForecastMainMake> getWrapper(ApsGoodsForecastMainMakeDto obj) {
    ...
    if (Objects.nonNull(obj)) {
      q.eq(Objects.nonNull(obj.getGoodsId()), ApsGoodsForecastMainMake::getGoodsId,
              obj.getGoodsId())
          .eq(StringUtils.isNoneBlank(obj.getForecastMakeMainNo()),
              ApsGoodsForecastMainMake::getForecastMakeMainNo, obj.getForecastMakeMainNo())
          .eq(StringUtils.isNoneBlank(obj.getForecastMakeMainName()),
              ApsGoodsForecastMainMake::getForecastMakeMainName, obj.getForecastMakeMainName())
          .eq(StringUtils.isNoneBlank(obj.getForecastMakeMainBeginDate()),
              ApsGoodsForecastMainMake::getForecastMakeMainBeginDate,
              obj.getForecastMakeMainBeginDate())
          .eq(StringUtils.isNoneBlank(obj.getForecastMakeMainEndDate()),
              ApsGoodsForecastMainMake::getForecastMakeMainEndDate,
              obj.getForecastMakeMainEndDate())
          .eq(Objects.nonNull(obj.getFactoryId()), ApsGoodsForecastMainMake::getFactoryId,
              obj.getFactoryId())
  ...
  public DynamicsPage<ApsGoodsForecastMainMakeQueryDataByIdRes> queryDataById(ApsGoodsForecastMainMakeQueryDataByIdReq req) {
    ...
    if (StringUtils.isNoneBlank(req.getBeginDate(), req.getEndDate())) {
      weekInfoList = calendarService.getWeekList(mainMake.getFactoryId(), req.getBeginDate(),
          req.getEndDate());
    } else {
      Calendar calendar = Calendar.getInstance();
      calendar.add(Calendar.MONTH, 6);
      weekInfoList = calendarService.getWeekList(mainMake.getFactoryId(),
          DateUtil.format(new Date(), "yyyy-MM"), DateUtil.format(calendar.getTime(), "yyyy-MM"));
    }

    List<ApsGoodsForecastMainMakeQueryDataByIdRes> resArrayList = new ArrayList<>();
    dataList.forEach((code, saleDataList) -> {
      ApsGoodsForecastMainMakeQueryDataByIdRes data = new ApsGoodsForecastMainMakeQueryDataByIdRes().setSaleConfigCode(
          code);
      data.put("saleConfigCode", code);
      weekInfoList.stream().collect(Collectors.groupingBy(t -> t.getCurrentDate().getYear()))
          .forEach((y, weekInfoListTmp) -> {
            saleDataList.stream().filter(t -> t.getSaleConfigCode().equals(code))
                .forEach(saleData -> {
                  weekInfoListTmp.stream().collect(Collectors.groupingBy(
                          t -> t.getCurrentDay().substring(5, 7) + "月" + t.getWeekNumber() + "周"))
                      .forEach((mw, weekInfoListTmpTmp) -> {
                        ...
                        weekInfoListTmpTmp.forEach(d -> {
                          Field field = getField(saleData,
                              "dayNum" + d.getCurrentDate().getDayOfYear());
    ...
    weekInfoList.stream().collect(Collectors.groupingBy(t -> t.getCurrentDate().getYear()))
        .forEach((y, weekInfoListTmp) -> {
          weekInfoListTmp.stream().collect(Collectors.groupingBy(
                  t -> t.getCurrentDay().substring(5, 7) + "月" + t.getWeekNumber() + "周"))
              .forEach((mw, weekInfoListTmpTmp) -> {
                headerList.add(new Header().setWidth(80).setFieldName(mw).setShowName(mw)
                    .setSortValue(weekInfoListTmpTmp.getFirst().getCurrentDay()));
              });
        });
    headerList.sort(Comparator.comparing(Header::getSortValue));
    headerList.addFirst(
        new Header().setWidth(80).setFieldName("saleConfigCode").setShowName("销售配置编码"));
    resArrayList.sort(
        Comparator.comparing(ApsGoodsForecastMainMakeQueryDataByIdRes::getSaleConfigCode));
    return new DynamicsPage<ApsGoodsForecastMainMakeQueryDataByIdRes>().setHeaderList(headerList)
        .setDataList(resArrayList);
  }
}
