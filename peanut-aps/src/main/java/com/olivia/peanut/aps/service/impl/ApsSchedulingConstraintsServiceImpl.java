...
public class ApsSchedulingConstraintsServiceImpl extends MPJBaseServiceImpl<ApsSchedulingConstraintsMapper, ApsSchedulingConstraints> implements
  ...
  private static List<Operator> operatorList(OperatorEnum... ignoreEnum) {
    ...
    if (ignoreEnum != null && ignoreEnum.length > 0) {
      ignoreSet.addAll(
          Arrays.stream(ignoreEnum).map(OperatorEnum::getValue).collect(Collectors.toSet()));
    }
    return Arrays.stream(OperatorEnum.values()).filter(t -> !ignoreSet.contains(t.getValue()))
  ...
  public @Override ApsSchedulingConstraintsQueryListRes queryList(ApsSchedulingConstraintsQueryListReq req) {
    ...
    List<ApsSchedulingConstraintsDto> dataList = list.stream()
        .map(t -> $.copy(t, ApsSchedulingConstraintsDto.class)).collect(Collectors.toList());
  ...
  public @Override DynamicsPage<ApsSchedulingConstraintsExportQueryPageListInfoRes> queryPageList(ApsSchedulingConstraintsExportQueryPageListReq req) {
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<ApsSchedulingConstraintsExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, ApsSchedulingConstraintsExportQueryPageListInfoRes.class));
    ...
    List<ApsSchedulingConstraintsExportQueryPageListInfoRes> listInfoRes = $.copyList(records,
  ...
  public ApsSchedulingConstraintsGetUseFieldRes getUseField() {
    ...
    FieldConfig createTimeField = new FieldConfig().setShowName("订单-创建时间")
        .setFieldName("createTime").setValueType(ValueType.DATE).setOperator(operator);
    FieldConfig deliveryDateField = new FieldConfig().setShowName("订单-交付时间")
        .setFieldName("deliveryDate").setValueType(ValueType.DATE).setOperator(operator);
    FieldConfig finishPayedDatetime = new FieldConfig().setShowName("订单-尾款支付时间")
        .setFieldName("finishPayedDatetime").setValueType(ValueType.DATE).setOperator(operator);
    FieldConfig updateTimeField = new FieldConfig().setShowName("订单-修改时间")
        .setFieldName("updateTime").setValueType(ValueType.DATE).setOperator(operator);
    FieldConfig orderTotalPrice = new FieldConfig().setShowName("订单-总价")
        .setFieldName("orderTotalPrice").setValueType(ValueType.TEXT).setOperator(operator);
    List<FieldConfig> values = List.of(createTimeField, deliveryDateField, finishPayedDatetime,
        updateTimeField, orderTotalPrice);
    List<ApsSaleConfigExportQueryPageListInfoRes> saleConfigList = apsSaleConfigService.queryPageList(
            new ApsSaleConfigExportQueryPageListReq().setQueryPage(false))
        .getDataList();

    List<FieldConfig> retList = new ArrayList<>(values);
    saleConfigList.forEach(sale -> {
      FieldConfig fieldConfig = new FieldConfig().setShowName("销售-" + sale.getSaleName())
          .setFieldName("sale_" + sale.getId()).setValueType(ValueType.SELECT)
          .setOperator(Operator.of(OperatorEnum.EQ, OperatorEnum.IN))
          .setValueItemList(sale.getChildren().stream()
              .map(c -> new ValueItem().setValue(c.getId()).setValueName(c.getSaleName()))
              .toList());
  ...
  public @Override void setName(
      List<? extends ApsSchedulingConstraintsDto> apsSchedulingConstraintsDtoList) {
  ...
  private MPJLambdaWrapper<ApsSchedulingConstraints> getWrapper(ApsSchedulingConstraintsDto obj) {
    ...
    if (Objects.nonNull(obj)) {
      q.eq(StringUtils.isNoneBlank(obj.getConstraintsNo()),
              ApsSchedulingConstraints::getConstraintsNo, obj.getConstraintsNo())
          .eq(StringUtils.isNoneBlank(obj.getConstraintsName()),
              ApsSchedulingConstraints::getConstraintsName, obj.getConstraintsName())
          .eq(StringUtils.isNoneBlank(obj.getConstraintsContext()),
              ApsSchedulingConstraints::getConstraintsContext, obj.getConstraintsContext())
          .eq(StringUtils.isNoneBlank(obj.getConstraintsRemark()),
              ApsSchedulingConstraints::getConstraintsRemark, obj.getConstraintsRemark())
