...
@Transactional
public class ApsSchedulingDayConfigVersionDetailServiceImpl extends
  ...
  public @Override ApsSchedulingDayConfigVersionDetailQueryListRes queryList(ApsSchedulingDayConfigVersionDetailQueryListReq req) {
    ...
    List<ApsSchedulingDayConfigVersionDetailDto> dataList = list.stream()
        .map(t -> $.copy(t, ApsSchedulingDayConfigVersionDetailDto.class))
  ...
  public @Override DynamicsPage<ApsSchedulingDayConfigVersionDetailExportQueryPageListInfoRes> queryPageList(
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      records = $.copyList(this.list(q),
          ApsSchedulingDayConfigVersionDetailExportQueryPageListInfoRes.class);
    }

    // 类型转换，  更换枚举 等操作

    List<ApsSchedulingDayConfigVersionDetailExportQueryPageListInfoRes> listInfoRes = $.copyList(
        records, ApsSchedulingDayConfigVersionDetailExportQueryPageListInfoRes.class);
    ...
    if (Objects.nonNull(obj)) {
      q
          .eq(Objects.nonNull(obj.getSchedulingDayId()),
              ApsSchedulingDayConfigVersionDetail::getSchedulingDayId, obj.getSchedulingDayId())
          .eq(StringUtils.isNoneBlank(obj.getConfigBizType()),
              ApsSchedulingDayConfigVersionDetail::getConfigBizType, obj.getConfigBizType())
          .eq(Objects.nonNull(obj.getConfigBizId()),
              ApsSchedulingDayConfigVersionDetail::getConfigBizId, obj.getConfigBizId())
          .eq(StringUtils.isNoneBlank(obj.getConfigBizName()),
              ApsSchedulingDayConfigVersionDetail::getConfigBizName, obj.getConfigBizName())
          .eq(Objects.nonNull(obj.getConfigBizNum()),
              ApsSchedulingDayConfigVersionDetail::getConfigBizNum, obj.getConfigBizNum())
          .eq(Objects.nonNull(obj.getOrderId()), ApsSchedulingDayConfigVersionDetail::getOrderId,
              obj.getOrderId())
          .eq(StringUtils.isNoneBlank(obj.getOrderNo()),
              ApsSchedulingDayConfigVersionDetail::getOrderNo, obj.getOrderNo())
          .eq(Objects.nonNull(obj.getIsMatch()), ApsSchedulingDayConfigVersionDetail::getIsMatch,
              obj.getIsMatch())
          .eq(Objects.nonNull(obj.getLoopIndex()),
              ApsSchedulingDayConfigVersionDetail::getLoopIndex, obj.getLoopIndex())
          .eq(Objects.nonNull(obj.getLoopEnough()),
              ApsSchedulingDayConfigVersionDetail::getLoopEnough, obj.getLoopEnough())
  ...
  private void setQueryListHeader(DynamicsPage<ApsSchedulingDayConfigVersionDetail> page) {
    ...
    tableHeaderService.listByBizKey(page,
        "ApsSchedulingDayConfigVersionDetailService#queryPageList");
