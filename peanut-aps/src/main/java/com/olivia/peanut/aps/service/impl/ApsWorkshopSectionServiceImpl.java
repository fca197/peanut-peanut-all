...
public class ApsWorkshopSectionServiceImpl extends MPJBaseServiceImpl<WorkshopSectionMapper, ApsWorkshopSection> implements
  ...
  public @Override WorkshopSectionQueryListRes queryList(WorkshopSectionQueryListReq req) {
    ...
    List<WorkshopSectionQueryListRes.Info> dataList = list.stream()
        .map(t -> $.copy(t, WorkshopSectionQueryListRes.Info.class)).collect(Collectors.toList());
  ...
  public @Override DynamicsPage<WorkshopSectionExportQueryPageListInfoRes> queryPageList(
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<WorkshopSectionExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, WorkshopSectionExportQueryPageListInfoRes.class));
    ...
    // 类型转换，  更换枚举 等操作
    List<WorkshopSectionExportQueryPageListInfoRes> listInfoRes = $.copyList(records,
  ...
  private MPJLambdaWrapper<ApsWorkshopSection> getWrapper(WorkshopSectionDto obj) {
    ...
    LambdaQueryUtil.lambdaQueryWrapper(q, obj, ApsWorkshopSection.class, ApsWorkshopSection::getId,
        ApsWorkshopSection::getSectionCode, ApsWorkshopSection::getSectionName);
