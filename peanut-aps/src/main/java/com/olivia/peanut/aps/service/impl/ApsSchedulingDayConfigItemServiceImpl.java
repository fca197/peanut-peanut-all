...
public class ApsSchedulingDayConfigItemServiceImpl extends MPJBaseServiceImpl<ApsSchedulingDayConfigItemMapper, ApsSchedulingDayConfigItem> implements
  ...
  public @Override ApsSchedulingDayConfigItemQueryListRes queryList(ApsSchedulingDayConfigItemQueryListReq req) {
    ...
    List<ApsSchedulingDayConfigItemDto> dataList = list.stream()
        .map(t -> $.copy(t, ApsSchedulingDayConfigItemDto.class)).collect(Collectors.toList());
  ...
  public @Override DynamicsPage<ApsSchedulingDayConfigItemExportQueryPageListInfoRes> queryPageList(ApsSchedulingDayConfigItemExportQueryPageListReq req) {
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<ApsSchedulingDayConfigItemExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, ApsSchedulingDayConfigItemExportQueryPageListInfoRes.class));
      records = dataList.getRecords();
    } else {
      records = $.copyList(this.list(q),
          ApsSchedulingDayConfigItemExportQueryPageListInfoRes.class);
    }

    // 类型转换，  更换枚举 等操作

    List<ApsSchedulingDayConfigItemExportQueryPageListInfoRes> listInfoRes = $.copyList(records,
        ApsSchedulingDayConfigItemExportQueryPageListInfoRes.class);
  ...
  public @Override void setName(
      List<? extends ApsSchedulingDayConfigItemDto> apsSchedulingDayConfigItemDtoList) {
    ...
    if (Objects.nonNull(obj)) {
      q
          .eq(Objects.nonNull(obj.getSchedulingDayId()),
              ApsSchedulingDayConfigItem::getSchedulingDayId, obj.getSchedulingDayId())
          .eq(Objects.nonNull(obj.getProcessId()), ApsSchedulingDayConfigItem::getProcessId,
              obj.getProcessId())
          .eq(Objects.nonNull(obj.getRoomId()), ApsSchedulingDayConfigItem::getRoomId,
              obj.getRoomId())
          .eq(Objects.nonNull(obj.getStatusId()), ApsSchedulingDayConfigItem::getStatusId,
              obj.getStatusId())
          .eq(StringUtils.isNoneBlank(obj.getConfigBizType()),
              ApsSchedulingDayConfigItem::getConfigBizType, obj.getConfigBizType())
          .eq(Objects.nonNull(obj.getConfigBizId()), ApsSchedulingDayConfigItem::getConfigBizId,
              obj.getConfigBizId())
          .eq(StringUtils.isNoneBlank(obj.getConfigBizName()),
              ApsSchedulingDayConfigItem::getConfigBizName, obj.getConfigBizName())
          .eq(Objects.nonNull(obj.getConfigBizNum()), ApsSchedulingDayConfigItem::getConfigBizNum,
              obj.getConfigBizNum())
          .eq(Objects.nonNull(obj.getConfigBizTime()), ApsSchedulingDayConfigItem::getConfigBizTime,
              obj.getConfigBizTime())
          .eq(Objects.nonNull(obj.getIsDefault()), ApsSchedulingDayConfigItem::getIsDefault,
              obj.getIsDefault())
