...
public class DictionaryServiceImpl extends MPJBaseServiceImpl<DictionaryMapper, Dictionary> implements DictionaryService {
  ...
  private MPJLambdaWrapper<Dictionary> getWrapper(DictionaryDto obj) {
    ...
    if (Objects.nonNull(obj)) {
      q.eq(Objects.nonNull(obj.getId()), Dictionary::getId, obj.getId())
          ...
          .eq(Objects.nonNull(obj.getVersionNum()), Dictionary::getVersionNum, obj.getVersionNum()).orderByDesc(Dictionary::getId);
    }
  ...
  private void setQueryListHeader(DynamicsPage<Dictionary> page) {
    page.addHeader("id", "id").addHeader("dictionaryGroup", "字典组").addHeader("dictionaryValue", "字典值").addHeader("dictionarySort", "排序")
        .addHeader("dictionaryExt", "而外信息").addHeader("tenantId", "所属租户id").addHeader("isDelete", "是否删除(0:否 1:是)").addHeader("createTime", "创建时间")
        .addHeader("createBy", "创建人id").addHeader("updateTime", "更新时间").addHeader("updateBy", "更新人id").addHeader("traceId", "链路追踪ID")
